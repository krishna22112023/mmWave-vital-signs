%function called for realtime wave form ploting of vital signs and data extraction (128 bits at a time) for signal post-proccessing and machine learning
% Created 05-Aug-2019 11:23
%
% This script assumes these variables are defined:
%
%   comportSnum - DATA port number
%   comportCliNum- UART port number
%   cliCfgFileName- Configuration file name with .cfg
%   loadCfg- If true, configuration file is loaded
%   app- GUI object

% This script assumes these functions are defined:

%   ECG - Preprocess ECG training data for arrhythmia prediction
%   neural_network_ecg - neural network model for training and testing
%   test- Test the trained neural network model
function [] = gui_components(comportSnum, comportCliNum, cliCfgFileName, loadCfg, app)

%Temperory integration of blood pressure monitoring
flag=0;
answer2 = questdlg('Detection option?', ...
	'Confirm selection', ...
	'Arhythmia detection','Blood pressure','Cancel');
% Handle response
switch answer2
    case 'Arhythmia detection'
        f = warndlg('Arhythmia detection selected','Confirmation');
    case 'Blood pressure'
        f = warndlg('Blood pressure detection selected','Confirmation');
        
        flag=flag+1; %set to 1 so that blood pressure functions are invoked
    case 'Cancel'
        PausePushed(app, event)
end
...
phase_unwrapped=zeros(5,PLOT_DISPLAY_LENGTH);
...
file_name = 'C:/Users/krishna/Desktop/DataOutput_XWR14xx';
fid_write_bin = fopen(strcat(file_name, '.txt'),'w');  % By Default open the file 
...
hLineHeartRate = plot(app.HeartRateWfm,NaN,'color','r'); %heart rate that needs to be extracted
hLinePhase = plot(app.PhaseUnwrapped,NaN,'color','k'); %chest displacement
...
%% Update Waveform Plots   
  ...
  %save variables to wrokspace for post-processing
     assignin('base','phase',outPhasePlot)
     assignin('base','breathwf',outBreathPlot)
     assignin('base','heartwf',outHeartPlot)
     
     
     phase_unwrapped=evalin('base','phase');
     breathing_data=evalin('base','breathwf');
     heartrate_data=evalin('base','heartwf');
     
%      heart_rate_array(temp1,temp2)=heart_rate;
%      
%      breath_rate_array(temp1,temp2)=breath_rate;
%      temp2=temp2+1;
     
     %Save data points 128 bits at a time 
     file='C:/Users/krishna/Desktop/heart_wave/out';
     fileID = fopen(strcat(file,'.txt'),'w');
     fprintf(fileID,'%4.4f \n',heartrate_data);
     fclose(fileID);
     
     file='C:/Users/krishna/Desktop/breathe_wave/out';
     fileID = fopen(strcat(file,int2str(file_count),'.txt'),'w');
     fprintf(fileID,'%4.4f \n',breathing_data);
     fclose(fileID);
     
     file='C:/Users/krishna/Desktop/phase_unwrapped/out';
     fileID = fopen(strcat(file,int2str(file_count),'.txt'),'w');
     fprintf(fileID,'%4.4f \n',phase_unwrapped);
     fclose(fileID);
     ...
     if(cnt==127) %check if 128 datapoints are stored and displayed
         %Blood pressure implementation : Data from second IWR EVM board
         %download uploaded neck based radar signal data from dropbox
         if(flag==1)
             assignin('base','heart_rate',heartRateEstDisplayFinal)
             assignin('base','breathing_rate',breathRateEstFFT)
             heart_rate=evalin('base','heart_rate');
             breath_rate=evalin('base','breathing_rate');
             file='C:/Users/krishna/Desktop/heart_rate/out';
             fileID = fopen(strcat(file,int2str(file_count),'.txt'),'w');
             fprintf(fileID,'%4.4f \n',heart_rate);
             fclose(fileID);
             file='C:/Users/krishna/Desktop/breath_rate/out';
             fileID = fopen(strcat(file,int2str(file_count),'.txt'),'w');
             fprintf(fileID,'%4.4f \n',breath_rate);
             fclose(fileID);
             key='xcwZwyCRl1gAAAAAAAACOObc29WbYVkpRPBmoE9zWb6Zv_40s2mThkDfm5DBaCe_';
             dropboxfile=strcat("out",".txt");
             store='C:/Users/krishna/Desktop/neck_data/';
             downloadFromDropbox(key,dropboxfile,store);
%              if file_count==1
             BloodPressureGUI
             pause(5);
%              end
         ...
         file_count=file_count+1   %keep a count of number of files saved. 1 file consists of 128 datapoints
%          temp1=temp1+1;
     end
     
%       arrhythmia detection and display corresponding metrics for 10 sets of observation data
     if file_count==11
         DiagnosisReport
         file_count=0;
